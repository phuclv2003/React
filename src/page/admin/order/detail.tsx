...
const OrderDetailAdmin: FC = () => {
  const location = useLocation();
  const [data] = useState<any>(location.state?.data);
  const [update] = useUpdateOrderMutation();

  const handleOrderDelivered = async (id: number, status: any, mes: string) => {
    try {
      await update({ id: id, status: status });
      message.success(mes);
    } catch (error) {
            ...
            <div className="flex pt-12">
                  ...
                  <div className="shadow-divider-1pt flex px-8 py-3 shadow-[0_0.1px_0_0] bg-white rounded-t-xl">
                    <div className="mr-auto flex max-w-[248px] flex-wrap leading-[1] md:flex md:max-w-[585px] md:flex-nowrap md:items-center">
                      <span className="css-1ktc22 text-text-primary line-clamp-1 max-w-[226px] md:max-w-[336px] font-semibold">
                        Đơn hàng {dayjs(data?.created_at).format("DD-MM-YYYY")}
                      </span>
              ...
              <div className="ml-1 px-3">
                <div className="bg-white p-4 rounded-t-xl">
                  ...
                  {data.state === 1 && (
                    <button
                      onClick={() => handleOrderDelivered(data.id, 2, "Đơn hàng đã được xác nhận")}
                      className="bg-[#1250dc] text-white px-4 py-2 mt-4 rounded-md"
                    >
                      Xác nhận đơn hàng
                    </button>
                  )}
                  ...
                  {data.state === 2 && (
                    <button
                    onClick={() => handleOrderDelivered(data.id, 3, "Đơn hàng đang giao")}
                      className="bg-[#1250dc] text-white px-4 py-2 mt-4 rounded-md"
                    >
                      Đang giao
                    </button>
                  ...
                  {data.state === 3 && (
                    <button
                    onClick={() => handleOrderDelivered(data.id, 4, "Đơn hàng giao thành công")}
                      className="bg-[#1250dc] text-white px-4 py-2 mt-4 rounded-md"
                    >
                      Đã giao
                    </button>
