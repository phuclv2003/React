...
import { useGetUserListCartsQuery, useUpdateQuantityCartsMutation } from "../../../services/cart";
...
const Cart: FC = () => {
  ...
  useEffect(() => {
    if (dataOrder) {
      const selectedItems = dataOrder.data.filter((item: any) => checkedItems.includes(item.product_id));
    ...
    if (checkedItems.length > 0) {
      ...
      const data = {
        products: selectedProducts.map(
          (product: { product_id: any; product_name: any; product_image: any; quantity: any; total_price: any;unit: string }) => ({
                    ...
                    <tr className="text-[#020b27] text-[13px] ">
                      <th className="text-left font-normal pl-4 py-4">
                        <Checkbox checked={selectAll}
                          onChange={handleSelectAll}>Chọn tất cả</Checkbox>
                      </th>
                      <th className="font-normal text-left">Giá thành</th>
                      <th className="font-normal whitespace-nowrap text-left">Số lượng</th>
                    ...
                    {dataOrder.data.map((item: any) => (
                      <tr className="py-2" key={item.product_id}>
                        <td className="flex gap-2 pl-4 py-4">
                          <Checkbox checked={checkedItems.includes(item.product_id)}
                            onChange={() => handleCheckboxChange(item.product_id)}></Checkbox>
                          <div>
                          ...
                          <div className="max-w-[280px]">{item.product_name}</div>
                        </td>
                        <td>{new Intl.NumberFormat("vi-VN").format(item.total_price / item.quantity)}
                          <span> đ</span></td>
                        <td className="whitespace-nowrap">
                          <div className="border-[1px] border-[#c1c8d1] w-max rounded-[34px] h-[32px] flex items-center">
                            <button
                              ...
                              onClick={() => Tru(item.product_id, item.quantity)}
                            >
                              -
                            </button>
                            <input
                            ...
                            <button
                              className="w-[35px] h-full"
                              onClick={() => Cong(item.product_id, item.quantity)}
                            >
                              +
                            </button>
                          </div>
              ...
              <div className="ml-1 px-3">
                <div className="bg-white p-4 rounded-t-xl">
                  <div className="flex justify-between">
                    <div>Tổng tiền</div>
                    <div>{new Intl.NumberFormat("vi-VN").format(totalPrice)}
                      <span> đ</span>
                    </div>
                  </div>
                  <div className="h-[1px] w-full bg-[#e4e8ed] mt-4"></div>
                  <div className="flex justify-between mt-3">
                    <div className="font-semibold text-[18px] text-[#020b27]">Tạm tính</div>
                    <div className="text-[20px] font-semibold text-[#1250dc]">{new Intl.NumberFormat("vi-VN").format(totalPrice)}
                      <span> đ</span>
                    </div>
                  </div>
                  <div>
                    <button onClick={() => order()} style={{
                      background: checkedItems.length <= 0 ? "#728091" : "linear-gradient(315deg,#1250dc 14.64%,#306de4 85.36%)",
                      cursor: checkedItems.length <= 0 ? "not-allowed" : "pointer"
                    }} className="w-full rounded-[42px] cursor-pointer outline-none p-3 mt-4 flex items-center justify-center text-white">Mua hàng {checkedItems.length > 0 ? `(${checkedItems.length})` : ""}</button>
                  </div>
        ...
        <div className="flex justify-center pt-5">
          <div className="text-center">
            <div className="mx-auto w-full">
              <img src="https://nhathuoclongchau.com.vn/estore-images/empty-cart.png" alt="" />
            </div>
            <h3 className="text-[#4a4f63] font-semibold text-[18px]">Chưa có sản phẩm nào trong giỏ hàng</h3>
            <p className="max-w-[300px] text-[#728091] text-[16px]">Cùng mua sắm hàng ngàn sản phẩm tại nhà thuốc FPT Long Châu nhé!</p>
            <div onClick={() => home()} className="flex justify-center mt-2 cursor-pointer">
              <div style={{ background: "linear-gradient(315deg,#1250dc 14.64%,#306de4 85.36%)" }} className="h-[48px] px-4 text-white rounded-[42px] flex justify-center items-center w-[60%]">
                Mua hàng
              </div>
            </div>
          </div>
